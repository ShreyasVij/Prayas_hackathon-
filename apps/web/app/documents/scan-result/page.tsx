...
function extractHeatmap(value: unknown): string | null {
  ...
  if (typeof obj.heatmap_url === "string" && obj.heatmap_url.startsWith("http")) return obj.heatmap_url;
  for (const v of Object.values(obj)) {
    if (v && typeof v === "object") {
      const nested = v as Record<string, unknown>;
      if (typeof nested.heatmap_image === "string" && nested.heatmap_image.length > 100) return nested.heatmap_image;
      if (typeof nested.heatmap_url === "string" && nested.heatmap_url.startsWith("http")) return nested.heatmap_url;
    }
  }
  return null;
}
...
export default function DocumentScanResultPage() {
  ...
  const view = useMemo(() => {
    ...
    const heatmapFromResponse = extractHeatmap(response);
    return {
    ...
    <div className="mx-auto max-w-6xl space-y-7 py-2">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        <div className="space-y-5 lg:col-span-5">
          ...
          {(heatmapUrl || view.heatmapFromResponse) ? (
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="relative flex aspect-square items-center justify-center overflow-hidden rounded-xl border border-orange-900 bg-zinc-950 shadow-md">
                  <img src={heatmapUrl || view.heatmapFromResponse!} alt="AI heatmap (Grad-CAM)" className="h-full w-full object-contain" />
